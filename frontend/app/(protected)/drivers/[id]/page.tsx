  ...
  const { data: profileData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.get(
        `/profiles/drivers/${id}?season=${season}`,
      );
        ...
        <div className="flex items-start gap-6">
          ...
          <div className="flex-1">
            ...
            <div className="mt-2 flex items-center gap-4">
              <TeamBadge teamName={currentSeason.team?.name || ""} size="md" />
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">
                {currentSeason.races_entered > 0
                  ? Math.round(
                      (currentSeason.wins / currentSeason.races_entered) * 100,
                    )
                  : 0}
                %
              </p>
            ...
            <div>
              ...
              <p className="text-2xl font-bold">
                {currentSeason.races_entered > 0
                  ? Math.round(
                      (currentSeason.podiums / currentSeason.races_entered) *
                        100,
                    )
                  : 0}
                %
      ...
      {profileData.career && profileData.career.seasons.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Last {profileData.career.seasons.length} seasons
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {profileData.career.total_wins}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">Total Points</p>
                  <p className="text-2xl font-bold">
                    {Math.round(profileData.career.total_points)}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">Championships</p>
                  <p className="text-2xl font-bold">
                    {profileData.career.championships}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">Seasons</p>
                  <p className="text-2xl font-bold">
                    {profileData.career.seasons.length}
                  </p>
                </div>
              </div>
                  ...
                  {profileData.career.seasons.map((seasonData: any) => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <TeamBadge teamName={seasonData.team || ""} size="xs" />
                      </div>
